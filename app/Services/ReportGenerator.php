...
class ReportGenerator
{
    ...
    private function doGenerate(Report $report): void
    {
        ...
        try {
            ...
            $response = Http::withHeaders([
                ...
                // single(심층 연애 리포트)의 max_tokens는 예전에 4096이었는데, 11개 섹션
                // 전체를 채우기엔 부족해서 종종 JSON이 완성되기 전에 잘렸다(stop_reason이
                // max_tokens) — 잘린 JSON은 마지막 키인 final_verdict가 아예 없는 상태로
                // 남아서 saveSingleJsonContent()의 스키마 검증에서 조용히 실패하고,
                // "큐 잡은 정상적으로 DONE 됐는데 리포트 내용은 저장 안 됨"처럼 보였다.
                // claude-sonnet-5는 최대 128k 출력 토큰을 지원하므로 8192는 여유 있는 값.
                'max_tokens' => $isSingle ? 8192 : 1200,
     ...
     *
     * (분량 축소 이력) 원래 love_os(6단계)/partner_view(5단계)/love_signal(5개 별도 섹션)이
     * 사실상 같은 "연애 단계별 흐름"을 세 번 다른 각도로 반복하고 있어서, max_tokens를
...
SCHEMA;
        ...
        return "당신은 사주 명리학과 연애 상담에 모두 능숙한 '결'의 연애 코치입니다. ".
            ...
            "9) 중복 방지 — love_profile/love_os/who_attracts_you/strength_weakness/recurring_pattern/partner_view/compatibility/relationship_advice/final_verdict는 서로 다른 질문에 답해야 합니다. 같은 내용을 다른 섹션에서 반복하지 마세요.\n".
            ...
            "12) 각 문장형 값은 1~3문장으로 간결하게, who_attracts_you.traits/love_os[*].signal 등 짧은 값은 5~20자 내외의 짧은 문구로 써서 전체 분량이 과도하게 길어지지 않게 하세요.";
    }
